...
export const createDeviceContext = async (req: Request, res: Response) => {
  ...
  const params = deviceContextParamsSchema.parse(req.params);
  const requestBody = createDeviceContextSchema.parse(req.body);
...
export const updateDeviceContext = async (req: Request, res: Response) => {
  const accountId = req.accountId;
  const params = deviceContextParamsSchema.parse(req.params);
  const requestBody = updateDeviceContextSchema.parse(req.body);
